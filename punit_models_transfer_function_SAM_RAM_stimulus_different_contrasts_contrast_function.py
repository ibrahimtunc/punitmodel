# -*- coding: utf-8 -*-
"""
Created on Wed Oct 28 15:01:33 2020
...
def foo(contrasts, axdims):
    """
    Same as punit_models_transfer_function_SAM_RAM_stimulus_different_contrasts.py but contrasts can be now changed
    inside the function
    
    Parameters
    ----------
    contrasts: 1-D array
        The array of contrasts to be checked
    axdims: array with 2 elements
        The array giving the axis dimensions to plot. for axdims=(n,m) the plot will be n*m
    """
    tlength = 100
    ...
    for cell_idx in range(len(parameters)):
        ...
        for cidx, contrast in enumerate(contrasts):
            ...
            for findex, fAM in enumerate(fAMs):
                ...
                #correctionfactor = 2.46902549 * 0.1220904473654484 #SAM stimulus power correction factor
        ...
        fig, axts = plt.subplots(*axdims, sharex=True, sharey='row')
        ...
        '''
        axts = np.delete(axts.reshape(np.product(axdims)), np.product(axdims)-1)
        ...
        for idx, ax in enumerate(axts):
            ...
            ax.set_title('contrast=%f' %(contrasts[idx]))
            ax2=ax.twinx()
        ...
        axresponse.plot(contrasts, RAMpowers[:,5], 'k.-', label='RAM response')
        axresponse.plot(contrasts, SAMpowers[:,5], 'r.-', label='SAM response')
        ...
        fig, axstimpowers = plt.subplots(*axdims, sharex=True, sharey=True)
        axstimpowers = np.delete(axstimpowers.reshape(np.product(axdims)), np.product(axdims)-1)
...
contrasts = np.linspace(0.02, 0.06, 5)
axdims = (3,2)
foo(contrasts,axdims)
